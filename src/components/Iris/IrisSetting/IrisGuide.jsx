...
import IrisSvg from "../../../assets/svgs/IrisSvg.svg";
...
const IrisGuide = () => {
  ...
  const handleStart = () => {
    // ws.send(msg);
    navigate("/iris-scan");
    //TODO: impl global logic
    setDisabled(true)
  };

  return (
    <div className="w-full h-1/2 flex justify-end">
      <div className="bg-white w-full px-4 pt-4 flex justify-between items-center rounded-md shadow-lg">
        <div className="text-darkBlue flex flex-col justify-between relative  space-y-2 w-2/5">
          ...
          <div className="flex items-start space-x-4 z-10 h-[4rem]">
            ...
            <div>
              ...
              <p className="text-xs text-dark">
                The software the decided weather the sample matchs any picture
        ...
        <div className="w-1/6">
          <img src={IrisSvg} alt="Illustration" />
        </div>
        <div className=" w-4/12 space-y-4 flex flex-col items-center">
          ...
          <p className="text-N50 text-xs text-center w-4/5">
            The processes are taking four major steps to identify and register
            the user.
          </p>
          <button
            className={`${
